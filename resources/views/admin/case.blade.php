...
<body class="flex flex-col h-screen justify-between">
    ...
    <x-content class="md:pb-20 md:px-20 pb-16 px-auto">
        <form action="{{ route('admin.case.entry') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-3 w-full h-full mt-10">
                ...
                <div class="">
                    <div class="flex justify-start items-center">
                        <h1 class="text-5xl text-orange-800 font-bold">Case</h1>
                    </div>
                    <div class="grid grid-rows-7 gap-5">
                        ...
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="case_size">Case Size</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="case_size" id="case_size" value="{{ old('case_size') }}">
                            @error('case_size')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                ...
                <div class="p-10">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="mt-8">
                            <input class="w-full h-auto border border-black mt-2" type="hidden" name="category" id="category" value="case">
                        </div>
                    </div>
                </div>
            </div>

...
<body class="flex flex-col h-screen justify-between">
    ...
    <x-content class="md:pb-20 md:px-20 pb-16 px-auto">
        <form action="{{ route('admin.cpu.cooler.entry') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-3 w-full h-full mt-10">
                ...
                <div class="">
                    <div class="flex justify-start items-center">
                        <h1 class="text-5xl text-orange-800 font-bold">CPU Cooler</h1>
                    </div>
                    <div class="grid grid-rows-7 gap-5">
                        ...
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="fan_size">Fan Size</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="fan_size" id="fan_size" value="{{ old('fan_size') }}">
                            @error('fan_size')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="fan_speed">Fan Speed</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="fan_speed" id="fan_speed" value="{{ old('fan_speed') }}">
                            @error('fan_speed')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="air_flow">AirFlow</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="air_flow" id="air_flow" value="{{ old('air_flow') }}">
                            @error('air_flow')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                ...
                <div class="p-10">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="mt-8">
                            <input class="w-full h-auto border border-black mt-2" type="hidden" name="category" id="category" value="cpu_cooler">
                        </div>
                    </div>
                </div>
            </div>

...
<body class="flex flex-col h-screen justify-between">
    ...
    <x-content class="md:pb-20 md:px-20 pb-16 px-auto">
        <form action="{{ route('admin.gpu.entry') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-3 w-full h-full mt-10">
                ...
                <div class="">
                    <div class="flex justify-start items-center">
                        <h1 class="text-5xl text-orange-800 font-bold">Graphics Card</h1>
                    </div>
                    <div class="grid grid-rows-7 gap-5">
                        ...
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="memory_type">Memory Type</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="memory_type" id="memory_type" value="{{ old('memory_type') }}">
                            @error('memory_type')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="memory_clock">Memory Clock</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="memory_clock" id="memory_clock" value="{{ old('memory_clock') }}">
                            @error('memory_clock')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="gpu_clock">GPU Clock</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="gpu_clock" id="gpu_clock" value="{{ old('gpu_clock') }}">
                            @error('gpu_clock')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                ...
                <div class="p-10">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="mt-8">
                            <input class="w-full h-auto border border-black mt-2" type="hidden" name="category" id="category" value="gpu">
                        </div>
                    </div>
                </div>
            </div>

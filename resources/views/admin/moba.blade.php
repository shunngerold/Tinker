...
<body class="flex flex-col h-screen justify-between">
    ...
    <x-content class="md:pb-20 md:px-20 pb-16 px-auto">
        <form action="{{ route('admin.moba.entry') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-3 w-full h-full mt-10">
                ...
                <div class="">
                    <div class="flex justify-start items-center">
                        <h1 class="text-5xl text-orange-800 font-bold">Motherboard</h1>
                    </div>
                    <div class="grid grid-rows-7 gap-5">
                        ...
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="cpu_socket">CPU Socket</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="cpu_socket" id="cpu_socket" value="{{ old('cpu_socket') }}">
                            @error('cpu_socket')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="ram_slots">RAM Slots</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="ram_slots" id="ram_slots" value="{{ old('ram_slots') }}">
                            @error('ram_slots')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="multiple_gpu">Support Multiple GPU</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="multiple_gpu" id="multiple_gpu" value="{{ old('multiple_gpu') }}">
                            @error('multiple_gpu')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                ...
                <div class="p-10">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="mt-8">
                            <input class="w-full h-auto border border-black mt-2" type="hidden" name="category" id="category" value="moba">
                        </div>
                    </div>
                </div>
            </div>

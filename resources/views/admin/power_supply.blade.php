...
<body class="flex flex-col h-screen justify-between">
    ...
    <x-content class="md:pb-20 md:px-20 pb-16 px-auto">
        <form action="{{ route('admin.power.supply.entry') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-3 w-full h-full mt-10">
                ...
                <div class="">
                    <div class="flex justify-start items-center">
                        <h1 class="text-5xl text-orange-800 font-bold">Power Supply</h1>
                    </div>
                    <div class="grid grid-rows-7 gap-5">
                        ...
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="form_factor">Form Factor</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="form_factor" id="form_factor" value="{{ old('form_factor') }}">
                            @error('form_factor')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="wattage">Wattage</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="wattage" id="wattage" value="{{ old('wattage') }}">
                            @error('wattage')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="efficiency_rate">Efficiency Rate</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="efficiency_rate" id="efficiency_rate" value="{{ old('efficiency_rate') }}">
                            @error('efficiency_rate')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="modularity">Modularity</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="modularity" id="modularity" value="{{ old('modularity') }}">
                            @error('modularity')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                ...
                <div class="p-10">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="mt-8">
                            <input class="w-full h-auto border border-black mt-2" type="hidden" name="category" id="category" value="power_supply">
                        </div>
                    </div>
                </div>
            </div>

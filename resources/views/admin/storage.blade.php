...
<body class="flex flex-col h-screen justify-between">
    ...
    <x-content class="md:pb-20 md:px-20 pb-16 px-auto">
        <form action="{{ route('admin.store.entry') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-3 w-full h-full mt-10">
                ...
                <div class="">
                    <div class="flex justify-start items-center">
                        <h1 class="text-5xl text-orange-800 font-bold">Storage</h1>
                    </div>
                    <div class="grid grid-rows-7 gap-5">
                        ...
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="storage_type">Storage Type</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="storage_type" id="storage_type" value="{{ old('storage_type') }}">
                            @error('storage_type')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="storage_capacity">Storage Capacity</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="storage_capacity" id="storage_capacity" value="{{ old('storage_capacity') }}">
                            @error('storage_capacity')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="transfer_speed">Transfer Speed</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="transfer_speed" id="transfer_speed" value="{{ old('transfer_speed') }}">
                            @error('transfer_speed')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                        <div>
                            <label class="text-lg text-orange-800 font-bold" for="cache_speed">Cache Speed</label>
                            <input class="w-full h-auto border border-black mt-2" type="text" name="cache_speed" id="cache_speed" value="{{ old('cache_speed') }}">
                            @error('cache_speed')
                                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                            @enderror
                        </div>
                ...
                <div class="p-10">
                    ...
                    <div class="flex items-center justify-center gap-4">
                        <div class="mt-8">
                            <input class="w-full h-auto border border-black mt-2" type="hidden" name="category" id="category" value="storage">
                        </div>
                    </div>
                </div>
            </div>
